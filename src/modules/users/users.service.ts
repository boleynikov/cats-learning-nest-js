...
export class UsersService {
    ...
    private async checkEmailUniqueness(email?: string, excludeUserId?: number): Promise<void> {
        if (!email) return;

        const existingUser = await this.userModel.findOne({ where: { email } });

        if (existingUser && existingUser.id !== excludeUserId) {
            throw new ConflictException({ message: EXCEPTION_MESSAGES.USER_WITH_EMAIL_ALREADY_EXISTS });
        }
    }
}
